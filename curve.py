    ...
    def make_plot(self, scx, scy):
        if self.is_changed:
            ...
            if self.ctype == 'bezier' and self.points_no > 0:
                self.plotpoints = [deCasteljau(self.points, t / plot_size)
                          for t in range(int(plot_size + 1))]
            elif self.ctype == 'rbezier' and self.points_no > 0:
                self.plotpoints = [deCasteljauRat(self.points, self.weights,
                                         t / plot_size)
                          for t in range(int(plot_size + 1))]
            else:
                self.plotpoints = []
            ...
            print("Plotpoints: ", self.plotpoints)
            print("Plotpointslen: ", len(self.plotpoints))
